...
def test_network(net, device, test_generator, verboise, request_Hausdorff_senz_spec=False):
    ...
    with torch.no_grad():
        for data in test_generator:
            ...
            decided_NCR = torch.where(predicted_NCR[:,:] > 0.5, decided_NCR, 0) # se verifica daca se trece de 0.5
            ...
            decided_ED = torch.where(predicted_ED[:,:] > 0.5, decided_ED, 0) # se verifica daca se trece de 0.5
            ...
            decided_ET = torch.where(predicted_ET[:,:] > 0.5, decided_ET, 0) # se verifica daca se trece de 0.5
